...
class ServerState:
    ...
    flush_every: int = 400              # Flush buffer to disk every N steps
    ...
    _image_bufs: Dict[str, List[np.ndarray]] = {}       # {cam_name: [obs_img, ...]}
    _image_next_bufs: Dict[str, List[np.ndarray]] = {}  # {cam_name: [next_obs_img, ...]}
    _pending_obs: Optional[np.ndarray] = None
    _pending_images: Dict[str, np.ndarray] = {}         # images waiting to be paired
    _file_idx: int = 0
...
def _record_step(obs_arr: np.ndarray, action_arr: np.ndarray, image_arrays: Dict[str, np.ndarray] = {}):
    """
    Append one transition to the buffer, including camera images.
    ...
    with state._lock:
        if state._pending_obs is not None:
            ...
            for cam_name, img in state._pending_images.items():
                if cam_name not in state._image_bufs:
                    state._image_bufs[cam_name] = []
                    state._image_next_bufs[cam_name] = []
                state._image_bufs[cam_name].append(img)
                cur_img = image_arrays.get(cam_name)
                state._image_next_bufs[cam_name].append(
                    cur_img.copy() if cur_img is not None else img.copy()
                )

        state._pending_obs = obs_arr.copy()
        state._pending_images = {k: v.copy() for k, v in image_arrays.items()}
...
def _flush_to_disk():
    ...
    Write the buffer to a compressed numpy file compatible with DictDemoDataset:
        {obs_key: array of shape (N, 2, state_dim)}
        {cam_name: array of shape (N, 2, H, W, 3)}  for each camera
    ...
    obs_arr = np.stack(state._obs_buf, axis=0)
    next_obs_arr = np.stack(state._next_obs_buf, axis=0)
    actions_arr = np.stack(state._action_buf, axis=0)

    paired_proprio = np.stack([obs_arr, next_obs_arr], axis=1)  # (N, 2, state_dim)

    save_data = {
        state.obs_key: paired_proprio,
        "actions": actions_arr,
    }

    for cam_name in state._image_bufs:
        imgs = np.stack(state._image_bufs[cam_name], axis=0)       # (N, H, W, 3)
        next_imgs = np.stack(state._image_next_bufs[cam_name], axis=0)
        save_data[cam_name] = np.stack([imgs, next_imgs], axis=1)  # (N, 2, H, W, 3)

    save_path = state.collect_dir / f"traj_{state._file_idx:04d}.npz"
    np.savez_compressed(save_path, **save_data)
    logger.info(
        f"Saved {len(state._obs_buf)} steps to {save_path} "
        f"(cameras: {list(state._image_bufs.keys()) or 'none'})"
    )

    state._obs_buf.clear()
    state._next_obs_buf.clear()
    state._action_buf.clear()
    state._image_bufs.clear()
    state._image_next_bufs.clear()
    state._file_idx += 1
...
async def plan(request: Request):
    ...
    # Record transition including camera images
    obs_np, image_arrays = _parse_observation(observation)
    _record_step(obs_arr=obs_np, action_arr=action_np, image_arrays=image_arrays)
